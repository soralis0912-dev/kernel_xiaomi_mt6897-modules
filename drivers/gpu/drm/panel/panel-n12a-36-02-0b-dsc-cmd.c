 ...
 * id it reads out of the command line, and takes the shorter one on the two ids
 * below. Keep the same split: the tables differ in the gamma block, so getting
 * it wrong shows up as a colour cast rather than a blank screen.
 */
#define BUILD_ID_P1_A			0x1110
#define BUILD_ID_P1_B			0x10
...
static bool panel_is_p1(void)
{
	return panel_build_id == BUILD_ID_P1_A || panel_build_id == BUILD_ID_P1_B;
}
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	if (panel_is_p1())
		...
		push_table(ctx, init_setting_p10, ARRAY_SIZE(init_setting_p10));
}
...
static void mode_switch_to(struct drm_panel *panel, int fps)
{
	...
	switch (fps) {
	case 60:
		table = panel_is_p1() ? mode_60hz_setting_p2 : mode_60hz_setting;
		count = panel_is_p1() ? ARRAY_SIZE(mode_60hz_setting_p2)
				      : ARRAY_SIZE(mode_60hz_setting);
		break;
	case 90:
		table = panel_is_p1() ? mode_90hz_setting_p2 : mode_90hz_setting;
		count = panel_is_p1() ? ARRAY_SIZE(mode_90hz_setting_p2)
				      : ARRAY_SIZE(mode_90hz_setting);
		break;
	case 120:
		table = panel_is_p1() ? mode_120hz_setting_p2 : mode_120hz_setting;
		count = panel_is_p1() ? ARRAY_SIZE(mode_120hz_setting_p2)
				      : ARRAY_SIZE(mode_120hz_setting);
		break;
	default:
		table = panel_is_p1() ? mode_144hz_setting_p2 : mode_144hz_setting;
		count = panel_is_p1() ? ARRAY_SIZE(mode_144hz_setting_p2)
				      : ARRAY_SIZE(mode_144hz_setting);
		break;
	}

	push_table(ctx, table, count);
	ctx->dynamic_fps = fps;
}
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "n12a_36_02_0b_dsc_cmd,lcm", },
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "panel-n12a-36-02-0b-dsc-cmd",
		.owner = THIS_MODULE,
...
MODULE_DESCRIPTION("n12a_36_02_0b_dsc_cmd oled panel driver");
